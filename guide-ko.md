...
여기서는 전체적인 오디오의 품질을 대폭 향상시켜주는 5종류의 필터에 대해 설명할 건데요, 설정이 전부 끝나면 이후에 누군가 오디오에 문제가 있다고 했을 때에도 수정을 하기가 쉬워질 겁니다.
...
> 피크 미터의 움직임이 사라질 때까지 폐쇄 역치값을 천천히 오른쪽으로 옮기세요. 말할 때에는 다시 움직임이 나타나야 합니다.
...
말하지 않을 때 마이크를 완전히 조용해지게 하거나, 배경 잡음을 더 효과적으로 제어하고 싶을 경우 편리합니다.

개방 준비/개방 유지/폐쇄 준비 시간은 따로 설정하지 않아도 될 수도 있습니다. 하지만 말 할 때 첫 음절이 잘려나간다면, 적어도 개방 준비 시간은 설정을 해두셔야 할 것입니다.
...
한가지 더! 음성 제한 필터의 임계값을 피크 미터에서 바로 조절할 수 있는 방법이 있습니다.

![피크 미터][OBS Peak Meter]

보통 음성 제한 필터는 채널에 가장 마지막으로 적용되는 필터입니다. 피크가 도달할 수 있는 최대값을 마지막으로 정해주거든요. 그리고 피크 미터의 볼륨 컨트롤은 마지막 필터에서 나온 마지막 신호의 볼륨을 조절합니다. 볼륨 컨트롤이 0.0dB로 설정되어 있으면, 이 마지막 신호는 그대로 유지됩니다.

<u>음성 제한 필터의 임계값을 0dB로 설정하면, 이 값은 볼륨 컨트롤과 동기화가 됩니다.</u> 기존의 볼륨 컨트롤은 설정 값을 초과하는 신호를 잘라버리므로, 소리가 뭉개집니다. 하지만 이 설정 값이 음성 제한 필터의 임계값인 경우에는 더 부드러운 방식으로 신호가 압축되므로 듣기에 더 좋은 소리가 나옵니다.
